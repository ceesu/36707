...
summary(fit)
pred <- predict(fit, data = spam.test, probability = TRUE)
...
```{r}
ftype <- c("linear", "polynomial","sigmoid", "radial")`for(k in type){
  fit <- svm(V58 ~ ., data = spam.train,
             kernel = k)
}
...
### (c) 
**ComparetheaccuracyyouhaveobtainedheretotheaccuracyyouobtainedonHomework 8 while using random forests**
