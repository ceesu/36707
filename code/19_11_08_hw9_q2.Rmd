...
We have shown the results for increasing fraction of Y=1 cases below for both logistic and svm classifiers. We see that as the fraction increases, the error decreases overall. The logistic classifier error gets lower for Y=1 cases than for Y=0 cases. This means that out of all of the errors made, missclassification of the Y=0 cases becomes more common.
...
for(i in length(frac)){
  ...
  model <- glm(Y ~X1+X2,data = new,family=binomial(link='logit'))
  pred <- predict(model, newdata = new_test)
  correct <- pred[pred != new_test$Y]
  #print(kable(table(new$Y, as.numeric(pred > 0.5))/length(pred)))
  lr_error_0[i] <- length(correct==0)/length(new_test$Y==0)
  lr_error_1[i] <- length(correct==1)/length(new_test$Y==1)
    
  # SVM
  fit <- svm(Y ~X1+X2, kernel= "radial", data = new)
  pred <- predict(fit, newdata = new_test)
  #print(kable(table(new$Y, pred)/length(pred)))
  correct <- pred[pred != new_test$Y]
  svm_error_0[i] <- length(correct==0)/length(new_test$Y==0)
  svm_error_1[i] <- length(correct==1)/length(new_test$Y==1)
}
...
The outputs (above) show that for the svm classifier, missclassification of the Y=0 cases becomes more common with increasing fraction. Furthermore, for a 95% fraction, all of the errors are made on the Y=0 data. 
...
Since SVMs find the decision boundary by 