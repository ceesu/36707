...
The decision boundary for this data is linear if we square both x1 and x2, since we know they are separated by a quadratic boundary.

We can get the nonlinear decision boundary through logistic regression, but it requires us to manually add the new covariates. Also as we see above the fit is not as good as with the SVM with radial kernel, so we need to invest additional time to tune the fit with these new covariates.
...
simulate <- function(fraction) {
  length= 10.0
  X1  <- matrix(runif(100, min=0, max=10),ncol=1)
  X2 <- matrix(runif(100, min=0, max=10),ncol=1)
...
pred <- predict.glm(model, newdata = new_test)
table(new$Y, pred)/length(pred)
```
...
SVMs find the decision boundary by 