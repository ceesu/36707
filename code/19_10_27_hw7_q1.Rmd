...
Based upon the summary statistics and the distributions seen in Figure \ref{fig:pairs}, we removed the following outlier teams:
...
We verified that this model could provide a good fit to the data by looking at the diagnostic plots. We found that 
...
Our EDA and variable selection had shown us that cortisol might be useful to include in our explanatory model of performance and so we additionally tested a model which includes cortisol levels and their interaction with diversity (Table X model X). However we found that in this model, neither cortisol nor its interaction with diversity are found to have a significant effect. 
