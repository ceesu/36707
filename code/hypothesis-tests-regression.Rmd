...
```{r}
c
generate_error_n <- function(X, sigma) {
...
1. It seems like the fraction of falsely rejected nulls does not increase with hte number of variables included, and the relationship is noisier rather than linear. In general as the number of variables increases, 
...
for (jj in N) {
  ...
  for (ii in 1:num_trials) {
    Y_tdist <- generate_error_t(X, sigma)*sqrt(jj) #scale t distribution so it has the same standard deviation 
    Y_ndist <- generate_error_n(X, sigma)
    fit_t <- lm(Y_tdist ~ ., data=X)
    fit_n <- lm(Y_ndist ~ ., data=X)
    
    pvals_tdist[ii] <- fit_t$
    rsq[jj] <- rsq[jj]+ summary(fit)$r.squared
    
  }
  # now make a plot of the pvals.
  d <- density(pvals_tdist)
  d2 <- density(pvals_ndist)
  
  plot(d)
  lines(d2)
  title(main = paste0("pval distribution for N=", N))
}
```
...
pvals_tdist <- numeric(length(N))
pvals_ndist <- numeric(length(N))
...
for (jj in N) {
  ...
  # extract f test pval from each of the fits. 
...
With p = 3 and N ranging from 1 to 500, I plotted the F-test pvalues for each fit (in the model with tdistributed errors, and in the model with normally distributed errors) on the same graph. As N increases, the hypothesis tests become more powerful as measured by teh number of significant p-values found. 

2. What do the effects of non-normality appear to be on the F test?

Looking at the same plot, it appears the pvalues from fitting teh t-distributed errors are 
