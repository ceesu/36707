...
For model selection we split the group level data 1:10  into a training and test set. Best subsets regression was performed on the training set with the leaps function regsubsets and predictions were assessed on the test set.
...
Since the authors studied 2-way interactions, to choose the terms in the model we first performed model selection using best subsets and cross validation. We start off with all of the group level variables as depicted in Figure \ref{fig:pairs}, adding on all of the two way interaction terms such as diversity.score:avg.log.testosterone to test these as well. To pick the number of terms we should 
...
x.test=model.matrix(final.performance~., data=team_dat[-train,vars])# notice the -index!
for(i in 1:36){
...
The training error decreases to zero when we get past about 10 terms. But the validation error seems lowest around 4 terms. Therefore we try to obtain the 
...
cv_errors = matrix(NA, k, 19, dimnames = list(NULL, paste(1:)))
...
```{r}
regfit_best = regsubsets(final.performance~.^2, data = team_dat[,vars], nvmax = 50)
coef(regfit_best, 4)
coef(regfit.fwd, 4)
```
