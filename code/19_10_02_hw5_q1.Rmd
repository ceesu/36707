...
```{r groups, fig.width=8, fig.height=2, warning= FALSE, echo= FALSE, message=FALSE, fig.cap="\\label{fig:uni} Univariate distribution of important group level variables in the analysis."}
...
For age, we have two variables: the variance of ages (age.variance) and the average age (avg.age) in the group. As seen in Figure\ref{fig:age} both however show very weak linear relationship with both testosterone and performance (r squared < 0.1 and < 0.01) respectively. 

```{r age, fig.width=8, fig.height=6, warning=FALSE, echo=FALSE, fig.cap="\\label{fig:age}Variance of age and average age of group members show very weak correlation with final performance and testosterone levels."}
...
Based on the above reasoning, we decided to keep discard both age and proportion of females as variables since they may not necessarily tell us more about the relationship between diversity, testosterone and performance. Furthermore this simplifies the model.
...
```{r model, fig.width=8, fig.height=4, warning=FALSE, echo=TRUE, fig.cap="Model results for testosterone and diversity alone."}
...
Next we fit a model to examine whether including an interaction between them could predict performance. We have controlled for team.size by including it in the model. It has the Residual standard error of 0.75 on 69 degrees of freedom.
...
We lastly fit a full model with both hormone measurements and their three way interaction with diversity.

```{r complete, fig.width=8, fig.height=4, warning=FALSE, echo=FALSE}
#  will stress change the effect of diversity score on performance?
