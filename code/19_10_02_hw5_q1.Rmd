...
knitr::opts_chunk$set(echo = FALSE,  width.cutoff=60)
# , fig.pos = 'H'
set.seed(1234)
...
To start we want to check the simplest assumptions from [@Akinola2018] that diversity score and testosterone do not significantly predict performance on their own. We use the F-test to compare this null hypothesis to the alternative hypothesis where the model only contains an intercept term (Table 1).

For these simple models, the coefficient of diversity score (0.066) and average log testosterone (0.703) are not  large in magnitude or significant (p >0.05) indicating that we do not reject the null hypothesis. This agrees with what the authors found.

```{r Q4 model, fig.width=4, fig.height=4, warning=FALSE, echo=FALSE, fig.cap="Model results for testosterone and diversity alone.", results='asis'}
mod_test_only = lm( final.performance ~avg.log.testosterone, data = team_dat)
mod_diversity_only = lm( final.performance ~diversity.score, data = team_dat)
...
full_mod1 <- lm( full_form1, data = team_dat)
full_mod2<- lm( full_form2, data = team_dat)
...
# library(pander)
# require(memisc)
# mtable123 <- mtable('Model_test_only' = lm0,
#             'Model_div_only' = lm1,
...
stargazer(
  #mod_test_only, mod_diversity_only, 
          full_mod1, 
          mod_sans_cortisol,
          full_mod2,
          model.names = FALSE,
          ...
          title = "Coefficients of regression models studied")
```
...
```{r test_model, fig.width=8, fig.height=8, warning=FALSE, echo=FALSE, fig.cap="Model results with interaction between diversity and performance"}
...
Figure \ref{fig:pairs} suggested a weakly linear relationship between cortisol and performance (pearson correlation coefficient of )
...
full_form2 <-as.formula(paste("final.performance ~ ", paste(vars, collapse= "+"), "+avg.log.testosterone:diversity.score",
                             ...
                             "+avg.log.testosterone:diversity.score:avg.log.cortisol", sep = ""))

mod_sans_cortisol <-lm( form_sans_cortisol, data = team_dat)
mod_sans_threeway <- lm( form_sans_cortisol , data = team_dat)
comparison_1 <- anova(full_mod2, mod_sans_cortisol)
kable(comparison_1, digits = 3, format = "pandoc", caption = "ANOVA table")
...
```{r stress, fig.width=8, fig.height=8, warning=FALSE, echo=FALSE}
