...
```{r bmi, include=FALSE, fig.width=5, fig.height=5, echo=FALSE, message=FALSE, warning= FALSE,fig.cap="\\label{fig:bmi} BMI corresponds to height and weight."}
test <- data$BMI/(data$weight/((data$height/100)^2))
...
p2 <-ggplot(data, aes(y=Age))+
  geom_boxplot() +theme_hw+ ylab("Age (years)")
p3 <-ggplot(data, aes(y=date))+
  geom_boxplot() +theme_hw+ ylab("Date (YYYYMMDD)")
p4 <-ggplot(data, aes(y= BMI))+
  geom_boxplot() +theme_hw
# summary(test)
...
## Current perceptions of checkup quality and the information recieved in checkups
...
p1 <- histogram(dat$quality_score, nint=5,  xlab = "Quality score")
p2 <- histogram(dat$info_score, nint=5, xlab = "Information score")
p3 <- histogram(dat$Wsttime, xlab = "Waste of time?")
p4 <- histogram(dat$Wstmon, xlab = "Waste of money?")
# dat$quality_score <- rowMeans(dat[,c(31:35)], na.rm = TRUE)
# dat$info_score <- rowMeans(dat[,c(42:45)], na.rm = TRUE)
...
```{r q2, fig.width=5, results='asis', fig.height=5, echo=FALSE, message=FALSE, warning= FALSE, fig.cap="\\label{fig:q2} Diagnostic plots for the model chosen by stepwise variable selection."}
...
```{r table, results='asis', fig.height=5, echo=FALSE, message=FALSE, warning= FALSE, fig.cap="\\label{fig:q2} Fit metrics for the model chosen by stepwise variable selection."}
# set.seed(8)
# library(bestglm)
...
```{r tune, fig.width=6, results='asis', fig.height=2, echo=FALSE, message=FALSE, warning= FALSE,fig.cap="\\label{fig:tune} Variable importance of tuned random forest classifier."}
...
optimal_ranger$variable.importance %>% 
  ...
  xlab("")+
  #labs(x="Importance (Gini index)")+
  geom_col() +
  coord_flip() +
  ggtitle("Top 10 important variables in Random Forest classifier")
```
...
opar=par(ps=8) 
histogram(new$predictions, main = "Predictions of time since last unprompted check-up", xlab = "")
```
