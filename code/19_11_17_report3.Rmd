...
data[,c(33:37,44:47)] <-lapply(data[,c(33:37,44:47)], as.character)
data[,c(33:37,44:47)] <-lapply(data[,c(33:37,44:47)], as.numeric)
...
```{r causal, fig.width=2, fig.height=1.5, echo=FALSE, fig.cap="\\label{fig:cause} Causal diagram illustrates hypothesized relationships between checkups, disease and cost of public healthcare."}
...
Here we want to predict the variable RecExam. 
We performed variable selection among all of the variables available (except the participant id) in order to exclude variables without large effects. First we picked how many terms we should have in the best predictive linear model by using 10-fold cross validation and plotted the mean squared error in Figure \ref{fig:cv}. This analysis shows validation error is lowest around 8 terms.
...
library(leaps)
set.seed(12)
folds=sample(rep(1:10,length=nrow(dat)))
...
for(k in 1:10){
  ...
  best.fit=regsubsets(RecPerExam~ ., data=dat[folds!=k, vars],nvmax=length(vars),
                      method = "forward") 
  
  #### ITERATE OVER EACH OF THE I NUMBER OF PREDICTORS
  for(i in 1:40){
    # get the fit on 'i' variables
    pred=predict.regsubsets(best.fit,dat[folds==k,vars], id=i)
    cv.errors[k,i]=mean((dat$RecExam[folds==k]-pred)^2)
  }
}
