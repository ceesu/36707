...
Yes, in fact the mse of the additive model is 2255.892 which is about half as much as the linear model.

```{r, warning=FALSE, results='asis'}
# Run the same baseline linear regression you used in Homework 6, so we have a basis to
# compare against. 
base=npregbw(ARR_DELAY~DEP_TIME +as.factor(ORIGIN) +
...
mean((test$ARR_DELAY - predict.gam(additive, newdata = test))^2, na.rm = TRUE)
```
...
If linear regression is appropriate, the additive model should have lower variance than we 

```{r, warning=FALSE, results='asis', fig.height=8}
# base plot
par(mfrow=c(3,2))
npplot(base, data =data, plot.errors.method = "bootstrap")
